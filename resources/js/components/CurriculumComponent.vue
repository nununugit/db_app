        ...
        <div class="row gx-5">
            <div v-for="(field, index) in fields.data" :key="index">
                ...
                <h2 class="fw-bolder mb-3">{{field.f_name}}</h2>
                    <div class="accordion-item" v-for="(curriculum, index_c) in sortField(curriculainfo,field.id)" :key="index_c">                    
                        <h3 class="accordion-header" v-bind:id="['heading-'+field.f_name+index_c]">
                            <button class="accordion-button" 
                            v-bind:class="{collapsed: activeItem !== field.f_name+index_c}"  
                            type="button" 
                            data-bs-toggle="collapse" 
                            v-bind:data-bs-target="['#collapse-'+field.f_name+index_c]" 
                            v-bind:aria-expanded="{true: activeItem === field.f_name+index_c}" 
                            v-bind:aria-controls="['collapse-'+field.f_name+index_c]" 
                            v-on:click="onActive(field.f_name+index_c)"
                            >
                                {{curriculum.c_name}}
                            </button>
                        </h3>
